        ...
        if files:
            ...
            for filename in files:
                if filename.lower().endswith(('.jpg', '.jpeg', '.png')) and filename in df['image'].tolist():
                    ...
                    x = df.loc[df['image'] == filename, 'x_by_percent'].values[0]
                    # (df.loc[df['image'] == filename, 'original_height'].values[0]) - 
                    y = (df.loc[df['image'] == filename, 'y_by_percent'].values[0])
                    width = df.loc[df['image'] == filename, 'value_width_percent'].values[0]
                    height = df.loc[df['image'] == filename, 'value_height_percent'].values[0]
